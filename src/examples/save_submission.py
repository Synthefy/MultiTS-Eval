"""
Save submission file in the required format for MUSEval competition.

This module provides functionality to convert results from MUSEval evaluation
...
from museval.data.benchmark import VERSION
import numpy as np


def save_submission(results: Dict[str, Any], output_dir: str, model_name: Optional[str] = None) -> None:
    """
    Save results in the submission format required by MUSEval competition.
    Creates separate JSON files for each model.
    
    Args:
        results: Results dictionary from MUSEval evaluation containing metrics
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Save MUSEval submission file")
